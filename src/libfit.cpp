...
struct FitPars {
  ...
  Float_t mu;
  Float_t A;
  const std::array<std::string, Nfpars> names = {"Q0", "sig0",  "Q1", "sig1",
                                                 "w",  "alpha", "mu", "A"};
};
...
struct Pts {
  ...
  std::vector<Int_t> *vi_Qpeak = new std::vector<Int_t>;
  std::vector<Int_t> *vi_Qvaley = new std::vector<Int_t>;
  std::vector<Int_t> *vi_dQpeak = new std::vector<Int_t>;
  std::vector<Int_t> *vi_dQvaley = new std::vector<Int_t>;
  Int_t i_Qpeak;
  Int_t i_Qvaley;
  Int_t i_Q0;
  Int_t i_Qend;
...
void findPeakValey(TH1F *h_Q0, TH1F *h_Qs, TH1F *h_dQ, Pts *pts,
  ...
  pts->g_dQpeaks->SetMarkerStyle(23);
  pts->g_dQpeaks->SetMarkerColor(3);
  pts->g_dQpeaks->SetMarkerSize(1);
  pts->g_dQvaleys->SetMarkerStyle(23);
  pts->g_dQvaleys->SetMarkerColor(4);
  ...
  pts->g_Qvaleys->SetMarkerColor(4);
...
PrepPars ParEst2(TH1D *h_Q, TH1D *h_Qs, PrepPars prepPars, Pts pts) {
  ...
  for (int i = 0; i < 5; ++i)
    std::cout << f_N->GetParName(i) << " = " << f_N->GetParameter(i)
              << std::endl;
  ...
  std::cout << "h_s parameters: \n";
  for (int i = 0; i < 5; ++i)
    std::cout << f_sig->GetParName(i) << " = " << f_sig->GetParameter(i)
              << std::endl;
...
PrepPars ParEst3(TH1F *h_Q, PrepPars prepPars, Pts pts) {
...
inline void SetPars(TF1 *f, PrepPars Pars, Bool_t printout = false) {
  ...
  for (int i = 0; i < f->GetNpar(); ++i) {
    std::cout << f->GetParName(i) << " = " << par[i] << std::endl;
    f->SetParameter(i, par[i]);
  }
}

void SetParLimFixs(TF1 *f, TH1F *h_Q, PrepPars Pars,
                   std::array<Bool_t, 8> fix = {0, 0, 0, 0, 0, 0, 0, 0},
                   std::array<Bool_t, 8> leave = {0, 0, 0, 0, 0, 0, 0, 0}) {
  Float_t Q0 = Pars.Q0;
  Float_t Q1 = Pars.Q1;
  Float_t Qv = Pars.Qv;
  Float_t Qp = Pars.Qp;
  ...
  Float_t mu = Pars.mu;
  Float_t A = Pars.A;
  std::array<std::array<Float_t, 2>, 9> l;
...
TFitResult *FitCompare(TH1F *h_Q, TF1 *f_Q, std::vector<PrepPars> v_prepPars) {
  int i_best = 0, i = 0;
  Double_t chi = 1e100;
  TFitResultPtr r;
  ...
  std::cout << "blafitres chi2 =" << r_best->Chi2() << std::endl;
...
void FitSPE(TDirectory *inputRootDir, TDirectory *outputRootDir,
  ...
  std::vector<Int_t> v_all[4];
  std::vector<Int_t> *v_Qp = &v_all[0], *v_Qv = &v_all[1], *v_dQp = &v_all[2],
                     *v_dQv = &v_all[3];
  ...
  std::vector<FitPars> v_FitPars;
  std::vector<PrepPars> v_prepPars;
  ...
  std::cout << h_Q0->GetBinLowEdge(1) * q_bin << std::endl;
  ...
  std::cout << "\tChi^2/NDF = " << BestFitResult.Chi2() / BestFitResult.Ndf()
            << std::endl;
  ...
  leg_fit->AddEntry("n",
                    ((std::string) "Chi^{2}/NDF = " +
                     std::to_string(BestFitResult.Chi2() / BestFitResult.Ndf()))
                        .data(),
                    "n");
  f_Q->Draw("same");
  leg_fit->Draw();
  ...
  c_fit->Print(
      addAppendixToRelativeFilePath("_fit.pdf", outputRootDir).c_str());
  ...
  for (int i = 0; i < BestFitResult.NPar(); ++i) {
    ...
    // std::cout << BestFitResult.GetParameterName(i) << ":" << parval.GetVal()
    // << std::endl;
